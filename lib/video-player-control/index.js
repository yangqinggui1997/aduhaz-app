...
export default class VideoPlayer extends Component {
    ...
    this.setState(
      ...
      () => {
        if (this.props.onPaused) {
          this.props.onPaused(!this.state.isPlaying);
        }
      },
    );
    this.showControls();
  ...
  pause() {
    ...
    if (this.props.hideControlsOnPaused) {
      this.hideControls();
    } else {
      this.showControls();
    }
  ...
  renderSeekBar(fullWidth) {
    const { customStyles, disableSeek, seekBarKnob } = this.props;
    return (
      <View
        style={[
          styles.seekBar,
        ...
        />
        {(!fullWidth && !disableSeek) || seekBarKnob ? (
          <View
            style={[
              styles.seekBarKnob,
      ...
      <View style={[styles.controls, customStyles.controls]}>
        <TouchableOpacity
          ...
          <Icon
            ...
            size={
              customStyles?.playIcon?.width ? customStyles?.playIcon?.width : 32
            }
          />
        </TouchableOpacity>
        {this.renderSeekBar()}
          ...
          <TouchableOpacity
            ...
            <Icon
              style={[
                styles.extraControl,
                customStyles.controlIcon,
                customStyles.volumeIcon,
              ]}
              name={this.state.isMuted ? 'volume-off' : 'volume-up'}
              size={
                customStyles?.fullIcon?.width
                  ? customStyles?.volumeIcon?.width
                  : 24
              }
            />
          </TouchableOpacity>
  ...
  renderVideo() {
    ...
    return (
      <View style={customStyles.videoWrapper}>
        ...
        ((!this.state.isPlaying && !this.props.hideControlsOnPaused) ||
